...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">Editar Programa</h1>
        <p class="text-primary-300">Modifica la información del programa seleccionado</p>
    </div>
    ...
    <div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
        ...
        <div class="bg-primary-800 bg-opacity-50 px-8 py-6 border-b border-primary-700">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h3 class="text-xl font-semibold text-white">Información del Programa</h3>
        ...
        <!-- Form Content -->
        <div class="p-8">
            <form method="POST" action="{{ route('programas.update', $programa->id_programa) }}" class="space-y-8">
                @csrf
                @method('PUT')

                <!-- Programa Information Section -->
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-primary-200 font-semibold mb-3">
                                <span class="flex items-center">
                                    <i class="fas fa-graduation-cap mr-2 text-accent-500"></i>
                                    Nombre del Programa
                        ...
                        <div>
                            ...
                            <select name="area_id" 
                                ...
                                <option value="" class="bg-primary-800">Selecciona un área</option>
                                @foreach($areas as $area)
                                    <option value="{{ $area->id_area }}" class="bg-primary-800" {{ old('area_id', $programa->area_id) == $area->id_area ? 'selected' : '' }}>
                                        {{ $area->nombre_area }} - {{ $area->sede->nombre_sede }}
                                    </option>
                                @endforeach
                            </select>
                            @error('area_id')
                        ...
                        <div>
                            ...
                            <select name="estado" 
                                    class="w-full bg-primary-800 border border-primary-600 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500 transition-colors duration-200" required>
                                <option value="1" class="bg-primary-800" {{ old('estado', $programa->estado) == '1' ? 'selected' : '' }}>Activo</option>
                                <option value="0" class="bg-primary-800" {{ old('estado', $programa->estado) == '0' ? 'selected' : '' }}>Inactivo</option>
                            </select>
                            @error('estado')
                                <p class="mt-2 text-red-400 text-sm flex items-center">
                                    <i class="fas fa-exclamation-triangle mr-2"></i>{{ $message }}
                ...
                <div class="border-t border-primary-700 pt-6 flex justify-end">
                    <a href="{{ route('programas.index') }}" class="bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-200 flex items-center mr-4">
