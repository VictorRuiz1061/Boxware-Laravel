...
@section('title', 'Editar Área | BoxWare')

@section('content')
<!-- Header Section -->
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">Editar Área</h1>
        <p class="text-primary-300">Actualiza la información del área en el sistema BoxWare</p>
    </div>
...
<div class="max-w-4xl mx-auto">
    <div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
        ...
        <div class="bg-primary-800 bg-opacity-50 px-8 py-6 border-b border-primary-700">
            <div class="flex items-center space-x-3">
                <div class="w-12 h-12 bg-accent-600 rounded-lg flex items-center justify-center">
                    <i class="fas fa-building-circle-check text-white text-lg"></i>
                </div>
                <div>
        ...
        <!-- Form Content -->
        <div class="p-8">
            <form method="POST" action="{{ route('areas.update', $area->id_area) }}" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="sede_id" 
                                ...
                                @foreach(\App\Models\Sede::all() as $sede)
                                    <option value="{{ $sede->id_sede }}" class="bg-primary-800" {{ old('sede_id', $area->sede_id) == $sede->id_sede ? 'selected' : '' }}>
                                        {{ $sede->nombre_sede }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="estado" 
                                    class="w-full bg-primary-800 border border-primary-600 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500 transition-colors duration-200" required>
                                <option value="1" class="bg-primary-800" {{ old('estado', $area->estado) == 1 ? 'selected' : '' }}>Activo</option>
                                <option value="0" class="bg-primary-800" {{ old('estado', $area->estado) == 0 ? 'selected' : '' }}>Inactivo</option>
                            </select>
                            @error('estado')
                                <p class="mt-2 text-red-400 text-sm flex items-center">
                                    <i class="fas fa-exclamation-triangle mr-2"></i>{{ $message }}
