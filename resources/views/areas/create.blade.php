...
<div class="max-w-xl mx-auto bg-white rounded-xl shadow-lg p-8">
    ...
    <form method="POST" action="{{ route('areas.store') }}">
        ...
        <div class="mb-4">
            ...
            <select name="id_sede" class="w-full border rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500" required>
    <option value="">Selecciona una sede</option>
    @foreach($sedes as $sede)
        <option value="{{ $sede->id_sede }}" {{ old('id_sede') == $sede->id_sede ? 'selected' : '' }}>{{$sede->nombre_sede }}</option>
    @endforeach
</select>
        </div>
