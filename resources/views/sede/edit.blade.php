...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">Editar Sede</h1>
        <p class="text-primary-300">Modifica la información de la sede seleccionada</p>
    </div>
    ...
    <div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
        ...
        <div class="bg-primary-800 bg-opacity-50 px-8 py-6 border-b border-primary-700">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h3 class="text-xl font-semibold text-white">Información de la Sede</h3>
        ...
        <!-- Form Content -->
        <div class="p-8">
            <form method="POST" action="{{ route('sede.update', $sede->id_sede) }}" class="space-y-8">
                @csrf
                @method('PUT')

                <!-- Sede Information Section -->
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="direccion_sede" 
                                   ...
                                   value="{{ old('direccion_sede', $sede->direccion_sede) }}" required>
                        ...
                        <div>
                            <label class="block text-primary-200 font-semibold mb-3">
                                <span class="flex items-center">
                                    <i class="fas fa-university mr-2 text-accent-500"></i>
                                    Centro
                                    <span class="text-red-400 ml-1">*</span>
                                </span>
                            </label>
                            <select name="centro_id" 
                                ...
                                <option value="" class="bg-primary-800">Selecciona un centro</option>
                                @foreach($centros as $centro)
                                    <option value="{{ $centro->id_centro }}" class="bg-primary-800" {{ old('centro_id', $sede->centro_id) == $centro->id_centro ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="estado" 
                                    class="w-full bg-primary-800 border border-primary-600 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500 transition-colors duration-200" required>
                                <option value="1" class="bg-primary-800" {{ old('estado', $sede->estado) == '1' ? 'selected' : '' }}>Activo</option>
                                <option value="0" class="bg-primary-800" {{ old('estado', $sede->estado) == '0' ? 'selected' : '' }}>Inactivo</option>
                            </select>
                            @error('estado')
                                <p class="mt-2 text-red-400 text-sm flex items-center">
                                    <i class="fas fa-exclamation-triangle mr-2"></i>{{ $message }}
                ...
                <div class="border-t border-primary-700 pt-6 flex justify-end">
                    <a href="{{ route('sede.index') }}" class="bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-200 flex items-center mr-4">
