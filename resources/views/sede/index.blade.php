...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">Gestión de Sedes</h1>
        <p class="text-primary-300">Administra las sedes del sistema BoxWare</p>
    </div>
    <a href="{{ route('sede.create') }}" 
...
<div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
    ...
    <div class="bg-primary-800 bg-opacity-50 px-8 py-6 border-b border-primary-700">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h3 class="text-xl font-semibold text-white">Lista de Sedes</h3>
                <p class="text-primary-300 text-sm">{{ count($sedes) }} sedes encontradas</p>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-primary-700">
                <thead>
                    <tr class="bg-primary-800 bg-opacity-40">
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold text-primary-300 uppercase tracking-wider">Dirección</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-primary-300 uppercase tracking-wider">Centro</th>
                ...
                <tbody class="divide-y divide-primary-700">
                    @forelse($sedes as $sede)
                        <tr class="hover:bg-primary-800 hover:bg-opacity-30 transition-colors duration-150">
                            <td class="px-6 py-4 text-white">{{ $sede->id_sede }}</td>
                            ...
                            <td class="px-6 py-4 text-primary-200">{{ $sede->direccion_sede }}</td>
                            <td class="px-6 py-4 text-primary-200">{{ $sede->centro->nombre_centro ?? 'N/A' }}</td>
                            <td class="px-6 py-4">
                                @if($sede->estado)
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    <a href="{{ route('sede.edit', $sede->id_sede) }}" 
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-primary-300">
                                <div class="flex flex-col items-center">
                                    <i class="fas fa-folder-open text-4xl mb-3 text-primary-600"></i>
                                    <p class="text-lg">No hay sedes registradas.</p>
                                    <p class="text-sm text-primary-400 mt-1">Crea una nueva sede para comenzar.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
