...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">Crear Elemento de Inventario</h1>
        <p class="text-primary-300">Agrega un nuevo elemento al inventario del sistema BoxWare</p>
    </div>
...
<div class="max-w-4xl mx-auto">
    <div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
        ...
        <!-- Form Content -->
        <div class="p-8">
            <form method="POST" action="{{ route('inventario.store') }}" class="space-y-8">
                @csrf
                ...
                <div>
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="descripcion" rows="5"
                                  class="w-full bg-primary-800 border border-primary-600 text-white placeholder-primary-400 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500 transition-colors duration-200"
                                  placeholder="Ingresa una descripción del elemento" required>{{ old('descripcion') }}</textarea>
                        @error('descripcion')
                            <p class="mt-2 text-red-400 text-sm flex items-center">
                                <i class="fas fa-exclamation-triangle mr-2"></i>{{ $message }}
                            </p>
