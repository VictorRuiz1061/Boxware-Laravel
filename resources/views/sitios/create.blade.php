...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">Crear Nuevo Sitio</h1>
        <p class="text-primary-300">Agrega un nuevo sitio al sistema BoxWare</p>
    </div>
...
<div class="max-w-4xl mx-auto">
    <div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
        ...
        <!-- Form Content -->
        <div class="p-8">
            <form method="POST" action="{{ route('sitios.store') }}" class="space-y-8">
                @csrf
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="nombre_sitio" 
                                   ...
                                   value="{{ old('nombre_sitio') }}" required>
                            @error('nombre_sitio')
                        ...
                        <div>
                            ...
                            <select name="tipo_sitio_id" 
                                ...
                                <option value="" class="bg-primary-800">Selecciona un tipo de sitio</option>
                                @foreach($tiposSitio as $tipoSitio)
                                    <option value="{{ $tipoSitio->id_tipo_sitio }}" class="bg-primary-800" {{ old('tipo_sitio_id') == $tipoSitio->id_tipo_sitio ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="estado" 
                                    class="w-full bg-primary-800 border border-primary-600 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500 transition-colors duration-200" required>
                                <option value="1" class="bg-primary-800" {{ old('estado', '1') == '1' ? 'selected' : '' }}>Activo</option>
                                <option value="0" class="bg-primary-800" {{ old('estado', '1') == '0' ? 'selected' : '' }}>Inactivo</option>
                            </select>
                            @error('estado')
                                <p class="mt-2 text-red-400 text-sm flex items-center">
                                    <i class="fas fa-exclamation-triangle mr-2"></i>{{ $message }}
                    ...
                    <div class="mt-6">
                        ...
                        <input type="date" name="fecha_tecnica" 
                                   class="w-full bg-primary-800 border border-primary-600 text-white placeholder-primary-400 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500 transition-colors duration-200"
                                   value="{{ old('fecha_tecnica', date('Y-m-d')) }}" required>
                        @error('fecha_tecnica')
                            <p class="mt-2 text-red-400 text-sm flex items-center">
                                <i class="fas fa-exclamation-triangle mr-2"></i>{{ $message }}
                            </p>
