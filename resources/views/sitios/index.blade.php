...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">Sitios</h1>
        <p class="text-primary-300">Administra los sitios del sistema BoxWare</p>
    </div>
...
<div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
    ...
    <div class="bg-primary-800 bg-opacity-50 px-8 py-6 border-b border-primary-700">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h3 class="text-xl font-semibold text-white">Lista de Sitios</h3>
                <p class="text-primary-300 text-sm">{{ count($sitios) }} sitios encontrados</p>
            </div>
        </div>
    </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="text-left text-primary-300 border-b border-primary-700">
                        ...
                        <th class="pb-4 font-semibold">Ubicación</th>
                        <th class="pb-4 font-semibold">Tipo de Sitio</th>
                ...
                <tbody>
                    @forelse ($sitios as $sitio)
                        <tr class="border-b border-primary-700 hover:bg-primary-800 hover:bg-opacity-50 transition-colors duration-200">
                            <td class="py-4 text-white">{{ $sitio->id_sitio }}</td>
                            <td class="py-4 text-white">{{ $sitio->nombre_sitio }}</td>
                            <td class="py-4 text-white">{{ $sitio->ubicacion }}</td>
                            <td class="py-4 text-white">{{ $sitio->tipoSitio->nombre_tipo_sitio }}</td>
                            <td class="py-4">
                                @if ($sitio->estado)
                            ...
                            <td class="py-4 text-right">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('sitios.edit', $sitio->id_sitio) }}" 
                        ...
                        <tr>
                            <td colspan="6" class="py-6 text-center text-primary-300">
                                <div class="flex flex-col items-center justify-center space-y-4">
                                    ...
                                    <a href="{{ route('sitios.create') }}" class="text-accent-500 hover:text-accent-400 transition-colors duration-200">
