...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">Editar Tipo de Movimiento</h1>
        <p class="text-primary-300">Modifica la información del tipo de movimiento seleccionado</p>
    </div>
...
<div class="max-w-4xl mx-auto">
    <div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
        ...
        <!-- Form Content -->
        <div class="p-8">
            <form method="POST" action="{{ route('tipo_movimiento.update', $tipoMovimiento->id_tipo_movimiento) }}" class="space-y-8">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="tipo_movimiento" 
                                   ...
                                   value="{{ old('tipo_movimiento', $tipoMovimiento->tipo_movimiento) }}" required>
                        ...
                        <div>
                            ...
                            <select name="estado" 
                                ...
                                <option value="1" class="bg-primary-800" {{ old('estado', $tipoMovimiento->estado) == '1' ? 'selected' : '' }}>Activo</option>
                                <option value="0" class="bg-primary-800" {{ old('estado', $tipoMovimiento->estado) == '0' ? 'selected' : '' }}>Inactivo</option>
                            </select>
                            @error('estado')
                                <p class="mt-2 text-red-400 text-sm flex items-center">
                                    <i class="fas fa-exclamation-triangle mr-2"></i>{{ $message }}
