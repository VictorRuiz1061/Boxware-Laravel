...
@section('title', 'Crear Tipo de Movimiento | BoxWare')

@section('content')
<<<<<<< HEAD
<div class="max-w-xl mx-auto bg-white rounded-xl shadow-lg p-8">
    <h2 class="text-2xl font-bold mb-6">Crear Tipo de Movimiento</h2>
    <form method="POST" action="{{ route('tipo_movimiento.store') }}">
        @csrf
        <div class="mb-4">
            <label class="block text-gray-700 font-semibold mb-2">Tipo de movimiento</label>
            <input type="text" name="tipo_movimiento" class="w-full border rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500" required>
        </div>
        <div class="mb-6">
...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">Crear Nuevo Tipo de Movimiento</h1>
        <p class="text-primary-300">Agrega un nuevo tipo de movimiento al sistema BoxWare</p>
    </div>
    <a href="{{ route('tipo_movimiento.index') }}" 
    ...
    <div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
        ...
        <div class="bg-primary-800 bg-opacity-50 px-8 py-6 border-b border-primary-700">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h3 class="text-xl font-semibold text-white">Información del Tipo de Movimiento</h3>
        ...
        <!-- Form Content -->
        <div class="p-8">
            <form method="POST" action="{{ route('tipo_movimiento.store') }}" class="space-y-8">
                @csrf

                <!-- Tipo Movimiento Information Section -->
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="tipo_movimiento" 
                                   ...
                                   placeholder="Ingresa el tipo de movimiento"
                                   value="{{ old('tipo_movimiento') }}" required>
                ...
                <div class="border-t border-primary-700 pt-6 flex justify-end">
                    <a href="{{ route('tipo_movimiento.index') }}" class="bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-200 flex items-center mr-4">
