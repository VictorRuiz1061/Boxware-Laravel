...
<div class="flex items-center mb-8">
    ...
    <div>
        <h1 class="text-3xl font-bold text-white mb-1">Crear Nueva Ficha</h1>
        <p class="text-primary-300">Registra una nueva ficha en el sistema</p>
    </div>
</div>
...
<div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
    ...
    <!-- Form Content -->
    <div class="p-8">
        <form action="{{ route('fichas.store') }}" method="POST" class="space-y-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="col-span-2">
                    <label for="id_ficha" class="block text-white font-medium mb-2">ID de la Ficha <span class="text-red-500">*</span></label>
                    <input type="text" id="id_ficha" name="id_ficha" value="{{ old('id_ficha') }}" 
                    ...
                    <div class="relative">
                        <select id="programa_id" name="programa_id" class="w-full bg-primary-800 bg-opacity-50 border border-primary-600 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-accent-500 appearance-none">
                            ...
                            @foreach($programas as $programa)
                                <option value="{{ $programa->id_programa }}" {{ old('programa_id') == $programa->id_programa ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-4">
                        <label class="inline-flex items-center">
                            <input type="radio" name="estado" value="1" class="form-radio h-5 w-5 text-accent-600 border-primary-600 bg-primary-800 bg-opacity-50 focus:ring-accent-500" {{ old('estado', '1') == '1' ? 'checked' : '' }}>
                            <span class="ml-2 text-white">Activa</span>
                        </label>
                        <label class="inline-flex items-center">
                            <input type="radio" name="estado" value="0" class="form-radio h-5 w-5 text-accent-600 border-primary-600 bg-primary-800 bg-opacity-50 focus:ring-accent-500" {{ old('estado') == '0' ? 'checked' : '' }}>
