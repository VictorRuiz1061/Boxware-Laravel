...
<div class="max-w-xl mx-auto bg-white rounded-xl shadow-lg p-8">
    ...
    <form method="POST" action="{{ route('fichas.store') }}">
        @csrf
        <div class="mb-4">
            <label class="block text-gray-700 font-semibold mb-2">Id ficha</label>
            <input type="text" name="id_ficha" class="w-full border rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500" required>
        </div>
        ...
        <div class="mb-4">
            <label class="block text-gray-700 font-semibold mb-2">Programa</label>
            <select name="programa_id" class="w-full border rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500">
                <option value="">Selecciona un programa</option>
                @foreach($programas as $programas)
                    <option value="{{ $programas->id_programa }}" {{ old('programa_id') == $programas->id_programa ? 'selected' : '' }}>{{ $programas->nombre_programa }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-4">
            ...
            <select name="usuario_id" class="w-full border rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500">
                ...
                @foreach($usuarios as $usuarios)
                    <option value="{{ $usuarios->id_usuario }}" {{ old('usuario_id') == $usuarios->id_usuario ? 'selected' : '' }}>{{ $usuarios->nombre }}</option>
                @endforeach
            </select>
        </div>
