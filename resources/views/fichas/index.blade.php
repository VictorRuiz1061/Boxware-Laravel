...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">Gestión de Fichas</h1>
        <p class="text-primary-300">Administra las fichas del sistema BoxWare</p>
    </div>
...
<div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
    ...
    <div class="bg-primary-800 bg-opacity-50 px-8 py-6 border-b border-primary-700">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h3 class="text-xl font-semibold text-white">Lista de Fichas</h3>
                <p class="text-primary-300 text-sm">{{ count($fichas) }} fichas encontradas</p>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-primary-700">
                <thead>
                    <tr class="bg-primary-800 bg-opacity-40">
                        <th class="px-6 py-4 text-left text-xs font-bold text-primary-300 uppercase tracking-wider">ID Ficha</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-primary-300 uppercase tracking-wider">Instructor</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-primary-300 uppercase tracking-wider">Programa</th>
                ...
                <tbody class="divide-y divide-primary-700">
                    @forelse($fichas as $ficha)
                        <tr class="hover:bg-primary-800 hover:bg-opacity-30 transition-colors duration-150">
                            <td class="px-6 py-4 text-white">{{ $ficha->id_ficha }}</td>
                            <td class="px-6 py-4 text-primary-200">
                                <div class="flex items-center">
                                    <i class="fas fa-user text-accent-500 mr-2"></i>
                                    <span>{{ $ficha->usuario->nombre ?? 'No asignado' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-primary-200">
                                <div class="flex items-center">
                                    <i class="fas fa-book text-accent-500 mr-2"></i>
                                    <span>{{ $ficha->programa->nombre_programa ?? 'No asignado' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                @if($ficha->estado)
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-8 text-center text-primary-300">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-lg">No hay fichas registradas.</p>
                                    <p class="text-sm text-primary-400 mt-1">Crea una nueva ficha para comenzar.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
