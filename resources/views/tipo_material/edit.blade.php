...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">Editar Tipo de Material</h1>
        <p class="text-primary-300">Modifica la información del tipo de material seleccionado</p>
    </div>
    <a href="{{ route('tipo_material.index') }}" 
    ...
    <div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
        ...
        <div class="bg-primary-800 bg-opacity-50 px-8 py-6 border-b border-primary-700">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h3 class="text-xl font-semibold text-white">Información del Tipo de Material</h3>
        ...
        <!-- Form Content -->
        <div class="p-8">
            <form method="POST" action="{{ route('tipo_material.update', $tipoMaterial->id_tipo_material) }}" class="space-y-8">
                @csrf
                @method('PUT')

                <!-- Tipo Material Information Section -->
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="tipo_elemento" 
                                   ...
                                   placeholder="Ingresa el tipo de elemento"
                                   value="{{ old('tipo_elemento', $tipoMaterial->tipo_elemento) }}" required>
                            @error('tipo_elemento')
                        ...
                        <div>
                            ...
                            <select name="estado" 
                                ...
                                <option value="1" class="bg-primary-800" {{ old('estado', $tipoMaterial->estado) == '1' ? 'selected' : '' }}>Activo</option>
                                <option value="0" class="bg-primary-800" {{ old('estado', $tipoMaterial->estado) == '0' ? 'selected' : '' }}>Inactivo</option>
                            </select>
                            @error('estado')
                                <p class="mt-2 text-red-400 text-sm flex items-center">
                                    <i class="fas fa-exclamation-triangle mr-2"></i>{{ $message }}
                ...
                <div class="border-t border-primary-700 pt-6 flex justify-end">
                    <a href="{{ route('tipo_material.index') }}" class="bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-200 flex items-center mr-4">
