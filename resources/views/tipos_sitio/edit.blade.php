...
@section('title', 'Editar Tipo de Sitio | BoxWare')

@section('content')
<<<<<<< HEAD
<div class="max-w-xl mx-auto bg-white rounded-xl shadow-lg p-8">
    <h2 class="text-2xl font-bold mb-6">Editar Tipo de Sitio</h2>
    <form method="POST" action="{{ route('tipos_sitio.update', $tipoSitio->id_tipo_sitio) }}">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label class="block text-gray-700 font-semibold mb-2">Nombre</label>
            <input type="text" name="nombre_tipo_sitio" class="w-full border rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500" required value="{{ old('nombre_tipo_sitio', $tipoSitio->nombre_tipo_sitio) }}">
        </div>
        <div class="mb-6">
            <label class="block text-gray-700 font-semibold mb-2">Estado</label>
            <select name="estado" class="w-full border rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500">
                <option value="1" {{ old('estado', $tipoSitio->estado) == 1 ? 'selected' : '' }}>Activo</option>
                <option value="0" {{ old('estado', $tipoSitio->estado) == 0 ? 'selected' : '' }}>Inactivo</option>
            </select>
        </div>
...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">Editar Tipo de Sitio</h1>
        <p class="text-primary-300">Modifica la información del tipo de sitio</p>
    </div>
...
<div class="max-w-4xl mx-auto">
    <div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
        ...
        <div class="bg-primary-800 bg-opacity-50 px-8 py-6 border-b border-primary-700">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-primary-300 text-sm">ID: {{ $tipo_sitio->id_tipo_sitio }}</p>
                </div>
            </div>
        </div>
        ...
        <!-- Form Content -->
        <div class="p-8">
            <form method="POST" action="{{ route('tipos_sitio.update', $tipo_sitio->id_tipo_sitio) }}" class="space-y-8">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="nombre_tipo_sitio" 
                                   ...
                                   value="{{ old('nombre_tipo_sitio', $tipo_sitio->nombre_tipo_sitio) }}" required>
                        ...
                        <div>
                            ...
                            <select name="estado" 
                                ...
                                <option value="1" class="bg-primary-800" {{ old('estado', $tipo_sitio->estado) == 1 ? 'selected' : '' }}>Activo</option>
                                <option value="0" class="bg-primary-800" {{ old('estado', $tipo_sitio->estado) == 0 ? 'selected' : '' }}>Inactivo</option>
                            </select>
                            @error('estado')
                                <p class="mt-2 text-red-400 text-sm flex items-center">
                                    <i class="fas fa-exclamation-triangle mr-2"></i>{{ $message }}
