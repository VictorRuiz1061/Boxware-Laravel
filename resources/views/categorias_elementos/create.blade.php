...
<<<<<<< HEAD
@section('title', 'Crear Categoría de Elemento | BoxWare')

@section('content')
<div class="max-w-xl mx-auto bg-white rounded-xl shadow-lg p-8">
    <h2 class="text-2xl font-bold mb-6">Crear Categoría de Elemento</h2>
    <form method="POST" action="{{ route('categorias_elementos.store') }}">
        @csrf
        <div class="mb-4">
            <label class="block text-gray-700 font-semibold mb-2">Código QNPSC</label>
            <input type="text" name="codigo_qnpsc" class="w-full border rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500" required>
        </div>
        <div class="mb-4">
            <label class="block text-gray-700 font-semibold mb-2">Nombre</label>
            <input type="text" name="nombre_categoria" class="w-full border rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500" required>
        </div>
        <div class="mb-6">
...
@endsection 
=======
@section('title', 'Crear Categoría | BoxWare')
...
<div class="flex items-center mb-8">
    ...
    <div>
        <h1 class="text-3xl font-bold text-white mb-1">Crear Nueva Categoría</h1>
        <p class="text-primary-300">Completa el formulario para crear una nueva categoría de elementos</p>
    </div>
</div>
...
<div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
    ...
    <!-- Form Content -->
    <div class="p-8">
        <form action="{{ route('categorias_elementos.store') }}" method="POST" class="space-y-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="codigo_qnpsc" value="{{ old('codigo_qnpsc') }}" class="w-full bg-primary-800 border border-primary-600 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500" placeholder="Ingrese el código">
                    @error('codigo_qnpsc')
                ...
                <div>
                    ...
                    <input type="text" name="nombre_categoria" value="{{ old('nombre_categoria') }}" class="w-full bg-primary-800 border border-primary-600 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500" placeholder="Ingrese el nombre">
                ...
                <div>
                    ...
                    <select name="estado" class="w-full bg-primary-800 border border-primary-600 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500">
                        <option class="bg-primary-800" value="1" {{ old('estado', '1') == '1' ? 'selected' : '' }}>Activa</option>
                        <option class="bg-primary-800" value="0" {{ old('estado') == '0' ? 'selected' : '' }}>Inactiva</option>
                    </select>
                    @error('estado')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
