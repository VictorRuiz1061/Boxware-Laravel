...
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="text-3xl font-bold text-white mb-2">Gestión de Centros</h1>
        <p class="text-primary-300">Administra los centros del sistema BoxWare</p>
    </div>
...
<div class="glass-effect rounded-xl border border-primary-700 overflow-hidden">
    ...
    <div class="bg-primary-800 bg-opacity-50 px-8 py-6 border-b border-primary-700">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h3 class="text-xl font-semibold text-white">Lista de Centros</h3>
                <p class="text-primary-300 text-sm">{{ count($centros) }} centros encontrados</p>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-primary-700">
                <thead>
                    <tr class="bg-primary-800 bg-opacity-40">
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold text-primary-300 uppercase tracking-wider">Municipio</th>
                ...
                <tbody class="divide-y divide-primary-700">
                    @forelse($centros as $centro)
                        <tr class="hover:bg-primary-800 hover:bg-opacity-30 transition-colors duration-150">
                            <td class="px-6 py-4 text-white">{{ $centro->id_centro }}</td>
                            ...
                            <td class="px-6 py-4 text-primary-200">
                                <div class="flex items-center">
                                    <i class="fas fa-map-marker-alt text-accent-500 mr-2"></i>
                                    <span>{{ $centro->municipio->nombre_municipio ?? 'No asignado' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                @if($centro->estado)
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    <a href="{{ route('centro.edit', $centro->id_centro) }}" 
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-8 text-center text-primary-300">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-lg">No hay centros registrados.</p>
                                    <p class="text-sm text-primary-400 mt-1">Crea un nuevo centro para comenzar.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
